...
__global__
void setup_kernel(curandState *state, int it) {
...
template <int n_dim>
__global__ void vegasFill(int n_strat, int n_intervals, int n_edges, int n_cubes, int it, int batch_size, int runs, int *counts, double *weights, double *x_edges, double *dx_edges, double *nh, double *JF, double *JF2, int *Counts, double *sum_r, double *sum_s, int *evals, curandState *dev_states) {
...
__global__
void results(int n_cubes, double v_cubes, double beta, int *Counts, double *JF, double *JF2, double *dh, double *nh, double *sum_r, double *sum_s) {
...
int main() {
    ...
    int max_eval_it = n_eval_it + 2 * n_cubes;
    int batch_size = max_eval_it < max_batch_size ? max_eval_it : max_batch_size;
    ...
    JF = (double*) malloc(n_cubes * sizeof(double));
    JF2 = (double*) malloc(n_cubes * sizeof(double));
    Counts = (int*) malloc(n_cubes * sizeof(int));
    ...
    checkCudaError(cudaMalloc(&JF_d, n_cubes*sizeof(double)));
    checkCudaError(cudaMalloc(&JF2_d, n_cubes*sizeof(double)));
    checkCudaError(cudaMalloc(&Counts_d, n_cubes*sizeof(int)));
    
    checkCudaError(cudaMalloc(&dev_states, batch_size*sizeof(curandState)));
    ...
    checkCudaError(cudaMalloc(&evals_d, max_eval_it*sizeof(int)));
    ...
    checkCudaError(cudaMemcpy(JF_d, JF, n_cubes*sizeof(double), cudaMemcpyHostToDevice));
    checkCudaError(cudaMemcpy(JF2_d, JF2, n_cubes*sizeof(double), cudaMemcpyHostToDevice));
    checkCudaError(cudaMemcpy(Counts_d, Counts, n_cubes*sizeof(int), cudaMemcpyHostToDevice));
    ...
    //init rng
    setup_kernel<<<gridSizeInit,blockSizeInit>>>(dev_states, batch_size);
    ...
    do {
        ...
        *dh_sum = 0.0;
        ...
        checkCudaError(cudaMemcpy(nh, nh_d, n_cubes*sizeof(double), cudaMemcpyDeviceToHost));
        ...
        checkCudaError(cudaMemcpy(evals_d, evals, idx*sizeof(int), cudaMemcpyHostToDevice));

        int runs = idx / batch_size + (int) ((idx % batch_size) != 0);

        //call kernel
        vegasFill<n_dim><<<gridSizeFill,blockSizeFill>>>(n_strat, n_intervals, n_edges, n_cubes, idx, batch_size, runs, counts_d, weights_d, x_edges_d, dx_edges_d, nh_d, JF_d, JF2_d, Counts_d, sum_r_d, sum_s_d, evals_d, dev_states);
        results<<<gridSizeRes,blockSizeRes>>>(n_cubes, v_cubes, beta, Counts_d, JF_d, JF2_d, dh_d, nh_d, sum_r_d, sum_s_d);
    ...
    checkCudaError(cudaFree(JF_d));
    checkCudaError(cudaFree(JF2_d));
    checkCudaError(cudaFree(Counts_d));
